...
class AudioViLDConfig:
    def __init__(self, mark_version="mark4.1"):  # change needed
        ...
        #    이 파일은 mark4.2(dragging)용이므로 아래 값만 mark4.2 기준이다.
        # [갱신 2026-08-02] mark4.2 재학습 후 0.45 -> 0.40 재선정 (dragging = 가구 끄는 소리).
        # val 430개(dragging 215 / others 215) 전 구간 스윕(0.05~0.95, 0.01 간격) 결과
        # accuracy 최고는 0.9581이고 0.40 / 0.46 / 0.52~0.53 세 곳이 동률이었음.
        # 그중 0.40을 채택한 근거는 혼동행렬이 유지되는 구간의 폭이다:
        #   0.40      -> (0.3923, 0.4044] 폭 0.0121   <- 가장 넓다
        #   0.46      -> (0.4580, 0.4633] 폭 0.0053
        #   0.52~0.53 -> (0.5205, 0.5268] 폭 0.0063
        # 0.40 성능: accuracy 0.9581, macro F1 0.9581, dragging P0.938 R0.981,
        #            others P0.980 R0.935, others FPR 0.065, TP211/FP14/FN4/TN201.
        # ⚠️ mark4.2는 동률 세 곳의 오류 성격이 뚜렷하게 다르다(accuracy는 같아도 무엇을 틀리는지가 다름).
        #    0.40 -> 놓침 4 / 오탐 14 (끄는 소리를 거의 다 잡지만 헛경보가 잦다)
        #    0.46 -> 놓침 6 / 오탐 12
        #    0.52 -> 놓침 9 / 오탐 9  (완전 대칭)
        #    헛경보를 줄이는 쪽이 중요해지면 0.52로 옮기는 것을 검토할 것. accuracy 손해는 없다.
        # 직전 0.45(acc 0.9558, 놓침 6 / 오탐 13)와 비교하면 놓침이 2개 줄고 오탐이 1개 는다.
        # 재현 검증: 저장된 예측을 thr=0.45로 재계산했을 때 430/430(100%) 일치,
        #            ROC AUC 자체 계산 0.9871 = 코랩 공식값 0.98708 -> 스윕 결과 신뢰 가능.
        # 근거 데이터: plots/threshold_sweep_mark4.2_val.csv (model/tune_threshold.py 산출)
